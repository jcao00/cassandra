...
public class StandardMemtable implements Memtable
{
    ...
    class FlushRunnable implements Callable<SSTableMultiWriter>
    {
        ...
        private void writeSortedContents()
        {
            if (logger.isDebugEnabled())
                logger.debug("Writing {}, flushed range = ({}, {}]", this.toString(), from, to);
            ...
            if (heavilyContendedRowCount > 0)
                logger.trace("High update contention in {}/{} partitions of {} ", heavilyContendedRowCount, toFlush.size(), this);
        }
    ...
    {
        if (region != Memtable.Region.ON_HEAP)
    ...
    {
        return region == Memtable.Region.ON_HEAP
