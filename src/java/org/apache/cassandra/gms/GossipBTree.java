...
public class GossipBTree
{
    ...
    GossipBTree(BTreeNode root, int size)
    {
        this.root = root;
        this.size = size;
    }
    ...
    public GossipBTree insert(Node node)
    {
        ...
        while (true)
        {
            ...
            if (comp == 0)
            {
                ...
                return new GossipBTree(newRoot, size);
            }
            else if (comp < 1)
            {
                if (curNode.left == null)
                {
                    ...
                    BTreeNode newRoot = buildNewTree(newBtreeNode, parents);
                    return new GossipBTree(newRoot, size + 1);
                }
                else
                {
    ...
    public <T> Iterator<T> iterator(Function<Node, T> function)
    {
        return new BTreeIterator<>(this, function);
    }
    ...
    class BTreeIterator<T> implements Iterator<T>
    {
        ...
        BTreeIterator(GossipBTree btree, Function<Node, T> function)
        {
