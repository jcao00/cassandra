...
public class OutboundMessagingConnection
{
     ...
     *
     * @return true if licking off the connection attempt was started by this thread; else, false.
     */
    public boolean connect()
    {
        ...
        logger.trace("connection attempt {} to {}", connectAttemptCount, connectionId);
        ...
        if (bootstrap == null)
        {
            ...
            bootstrap = buildBootstrap(compress);
        }

        ChannelFuture connectFuture = bootstrap.connect();
        connectFuture.addListener(this::connectCallback);

        long timeoutMills = Math.max(MINIMUM_CONNECT_TIMEOUT_MS, DatabaseDescriptor.getRpcTimeout());
    ...
    boolean connectCallback(Future<? super Void> future)
    {
        ...
        if (cause instanceof IOException)
        {
            ...
            connectAttemptCount++;
            scheduledExecutor.schedule(this::connect, OPEN_RETRY_DELAY_MS * connectAttemptCount, TimeUnit.MILLISECONDS);
        }
        else
        {
    ...
    {
        // TODO:JEB should we ignore any callback if the state is CLOSED?
        // what about state != CREATING_CHANNEL ??
