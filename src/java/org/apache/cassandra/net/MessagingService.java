...
public final class MessagingService implements MessagingServiceMBean
{
    ...
    private void blockForPeers(int targetAlivePercent, int aliveTimeoutSecs)
    {
        ...
        Set<Map.Entry<InetAddress, EndpointState>> peers = new HashSet<>(Gossiper.instance.getEndpointStates());

        // remove current node from the set
        peers = peers.stream()
                                 .filter(entry -> !entry.getKey().equals(FBUtilities.getBroadcastAddress()))
        ...
        {
            int currentAlive = 0;
            for (Map.Entry<InetAddress, EndpointState> entry : peers)
            {
    ...
    private AtomicInteger sendPingMessages(Set<Map.Entry<InetAddress, EndpointState>> peers)
    {
        ...
        for (Map.Entry<InetAddress, EndpointState> peer : peers)
