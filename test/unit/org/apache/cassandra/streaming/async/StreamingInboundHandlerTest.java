...
public class StreamingInboundHandlerTest
{
    ...
    {
        StreamInitMessage msg = new StreamInitMessage(REMOTE_ADDR.getAddress(), 0, UUID.randomUUID(), "desc", true, true, UUID.randomUUID());
    ...
    {
        return new StreamSession(sid.from, sid.from, (connectionId, protocolVersion) -> null, sid.sessionIndex, true, true, UUID.randomUUID());
    }
    ...
    public void StreamDeserializingTask_deriveSession_IFM_NoSession() throws InterruptedException, IOException
    {
        FileMessageHeader header = new FileMessageHeader(TableId.generate(), REMOTE_ADDR.getAddress(), UUID.randomUUID(), 0, 0,
                                                         BigFormat.latestVersion, SSTableFormat.Type.BIG, 0, new ArrayList<>(), null, 0, 0, null);
    ...
    public void StreamDeserializingTask_deriveSession_IFM_HasSession() throws InterruptedException, IOException
    {
        ...
        StreamResultFuture future = StreamResultFuture.initReceivingSide(0, planId, "desc", REMOTE_ADDR.getAddress(), channel, true, true, UUID.randomUUID());
