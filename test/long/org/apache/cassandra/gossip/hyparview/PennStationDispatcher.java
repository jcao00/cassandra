...
public class PennStationDispatcher implements IMessageSink
{
    ...
    {
        peers.put(addr, new NodeContext(addr, datacenter, seedProvider));
    }
    ...
    public boolean allowOutgoingMessage(MessageOut message, int id, InetAddress to)
    {
        if (containsNode(to))
        {
            ...
            peers.get(to).send((HyParViewMessage)message.payload, 0);
        }
        return false;
    }
    ...
    public void awaitQuiesence()
    {
        ...
        Uninterruptibles.sleepUninterruptibly(peerCount, TimeUnit.MILLISECONDS);

        int sleepTime = peerCount < 50 ? 10 : (int)Math.ceil(peerCount / 10) ;
        final int quiescentRounds = 6;
        int count = 0;
        while (true)
        {
            if (currentlyProcessingCount.get() == 0)
            {
                count++;
                if (count == quiescentRounds)
                    return;
            }
            else
            {
    ...
    public class NodeContext
    {
        ...
        protected NodeContext(InetAddress addr, String datacenter, SeedProvider seedProvider)
        {
            ...
            hpvService = new HyParViewService(addr, datacenter, seedProvider, scheduler, scheduler, 3);
